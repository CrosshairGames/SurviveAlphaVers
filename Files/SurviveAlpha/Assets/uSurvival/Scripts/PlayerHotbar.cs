...
public class PlayerHotbar : NetworkBehaviour, ICombatBonus
{
    ...
    [SyncVar] public int selection = 0; // always between 0 and slots.count, no checks needed
    ...
    {
        // reset usage time for local player
        if (isLocalPlayer)
            usageEndTime = NetworkTime.time + itemData.cooldown;
    ...
    public void CmdUseItem(int index, Vector3 lookAt)
    {
            ...
            CheckDurability(index))
        {
            ...
            else
            {
                ...
                Debug.LogWarning("CmdUseItem rejected for: " + name + " item=" + itemData.name + "@" + NetworkTime.time);
            }
        }
    }
