...
public class PlayerLook : NetworkBehaviour
{
    ...
    [SyncVar, HideInInspector] bool syncedIsFreeLooking;
    ...
    public Vector3 lookPositionRaycasted
    {
        get
        {
            ...
            else
            {
                ...
                //return Utils.RaycastWithout(firstPersonParent.position, syncedLookDirectionRaycasted, out hit, Mathf.Infinity, gameObject)
                //       ? hit.point
                //       : lookPositionFar;
    ...
    static void CompressDirection(Vector3 direction, out byte xRotation, out byte yRotation)
    {
        ...
        xRotation = Utils.ScaleFloatToByte(euler.x, 0, 360, byte.MinValue, byte.MaxValue);
        yRotation = Utils.ScaleFloatToByte(euler.y, 0, 360, byte.MinValue, byte.MaxValue);
    }

    static Vector3 UncompressDirection(byte xRotation, byte yRotation)
    {
        // uncompress the bytes
        float x = Utils.ScaleByteToFloat(xRotation, byte.MinValue, byte.MaxValue, 0, 360);
        float y = Utils.ScaleByteToFloat(yRotation, byte.MinValue, byte.MaxValue, 0, 360);
    ...
    void Update()
    {
        ...
        if (Time.time - lastClientSendTime >= syncInterval)
        {
            ...
            byte x, y = 0;
            CompressDirection(lookDirectionFar, out x, out y);
