...
public class StructureItem : UsableItem
{
    ...
    {
        // get components
        Construction construction = hotbar.GetComponent<Construction>();
        ...
        */
        ...
        return CanBuildThere(look.headPosition, bounds)
               ? Usability.Usable
    ...
    public virtual bool CanBuildThere(Vector3 headPosition, Bounds bounds)
    {
        // nothing at this position yet?
        // => we check 90% of size so things can at least barely touch each other
        if (Physics.CheckBox(bounds.center, bounds.extents * (1 - buildToleranceCollision)))
            return false;

        // not floating in air?
        // => needs to touch anything else if 110% of collider
        if (!Physics.CheckBox(bounds.center, bounds.extents * (1 + buildToleranceAir)))
            return false;

        // linecast to make sure that nothing is between us and the build preview
        return !Physics.Linecast(headPosition, bounds.center);
    }
